...
# 中断
	...
	* 内部中断
		* 软中断
		* 异常(故障Fault，异常Trap，终止Abort)
	* 中断向量表(IVT，BIOS执行简单初始化过程会建立IVT和ISR)
		* 映射不同中断与不同中断服务程序,通过中断向量表，获取中断服务程序(ISR)入口地址
		* 起始于物理地址0,长度为1KB
		* 每个单元4字节，连续256个单元
		* 每个单元存放一个中断服务程序的入口地址
	* 保护模式下中断处理
		* 使用中断描述符表(IDT)
			* 每个元素8字节，即中断描述符
			* 使用前加载图IDTR,lidt
			* 每个中断产生一个中断向量，通过IDT查找对应的中断描述符，在通过中断描述符中的选择子和偏移量找到ISR入口地址
			* 中断门描述符(Type 1110,中断门与陷阱门(1111)Param不使用)
			|				16 - 31			|	15	|	13-14	|	12	|	8-11	| 7 | 6 | 5 | 0-4   |
			|			中断服务程序偏移地址2 	|	P 	|	DPL		|	S	|	Type	| 0 | 0 | 0 | Param |  高32位
			
			|				16 - 31			|							 0 - 15							|
			|			中断服务程序段选择子	|					中断服务程序偏移地址1						|  低32位
		
# 外设与CPU中断通信
	* 使用8259A中断代理
	* 通过变成对8259A进行功能配置(内核工作):屏蔽外设中断，对中断优先级进行裁决、向处理器提供中断向量
	* 组成：
		* INT  ：选出优先级最高的中断请求，发信号通知CPU
		* INTA ：中断响应信号，接受来自CPU,INTA接口的中断响应信号
		* PR   ：优先级仲裁器，多中断，找出优先级最高
		* IMR  ：中断屏蔽寄存器，屏蔽某个外设中断
		* ISR  ：中断服务寄存器，某中断正在被处理，保存在此寄存器
		* IRR  ：中断请求寄存器，接受经IMR过滤后的中断信号并锁存，存的都是待处理中断
	* 工作方式：
		* 触发方式 ：边沿(上升沿,推荐)，电平变化瞬间认为中断申请到来; 电平，引脚信号稳定一段时间，认为中断申请到来
		* 数据连接方式 ：非缓冲，直接与数据总线相连; 缓冲，通过总线驱动器与数据总线相连
		* 中断优先级 ： 固定，IR0-IR7; 自动循环，响应后，中断源优先级降为最低; 特殊循环，变成指定某中断源位最低，其他自动改变
		* 中断嵌套 ：完全嵌套，中断服务程序执行期间，不响应本级和较低级中断; 特殊完全嵌套，响应本级但不响应低级
		* 屏蔽方式 ：普通，将IMR某一位或几位置1屏蔽对应级别中断; 特殊， 未屏蔽的中断源均可被中断服务程序响应即低优先级可打断高优先级
		* 中断结束 ：自动(非多重中断)，8259A自动清除ISR已置位的优先级最高的位; 手动,中断服务程序结束，向8259A发中断结束命令，清除ISR相应位
